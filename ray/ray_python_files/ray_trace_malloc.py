...
def shortest_way(Location, n):
    ...
    for i in range(0, n):
        for j in range(0, n):
            dis[i][j] = (Location[2 * i] - Location[2 * j]) * (Location[2 * i] - Location[2 * j]) + (
                        Location[2 * i + 1] - Location[2 * j + 1]) * (Location[2 * i + 1] - Location[2 * j + 1])
    nums = []
    for i in range(0, n):
        nums.append(i)
    print(nums)
    for num in itertools.permutations(nums, n):  # 逐个生成各个点序号的全排列
        ...
        for j in range(0, n - 1):
            route_dis = route_dis + (Location[2 * num[j]] - Location[2 * num[j + 1]]) * (
                        Location[2 * num[j]] - Location[2 * num[j + 1]]) + (
                                    Location[2 * num[j] + 1] - Location[2 * num[j + 1] + 1]) * (
                                    Location[2 * num[j] + 1] - Location[2 * num[j + 1] + 1])
        print(route_dis)
